...
if __name__ == "__main__":
    fnames = sorted([fname for fname in os.listdir("pubmed/baseline") if fname.endswith("xml.gz")])
    
    if not os.path.exists("pubmed/chunk"):
        os.makedirs("pubmed/chunk")

    for fname in tqdm.tqdm(fnames):
        if os.path.exists("pubmed/chunk/{:s}".format(fname.replace(".xml.gz", ".jsonl"))):
            continue
        gz_fpath = os.path.join("pubmed/baseline", fname)
        ...
        with open("pubmed/chunk/{:s}".format(fname.replace(".xml.gz", ".jsonl")), 'w') as f:
            f.write('\n'.join(saved_text))